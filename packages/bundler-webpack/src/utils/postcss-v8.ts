...
export default class PostcssConfig {
  ...
  configFromFile() {
    const loaderConfig =
      (this.postcssOptions && this.postcssOptions.config) || {};
  ...
  normalize(postcssOptions) {
    ...
    if (Array.isArray(postcssOptions)) {
      consola.warn(
        'Using an Array as `build.postcss` will be deprecated in Nuxt 3. Please switch to the object' +
          ' declaration',
      );
  ...
  sortPlugins({ plugins, order }) {
    ...
    return typeof order === 'function'
      ? order(names, orderPresets)
      : order || names;
  }
