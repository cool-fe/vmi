...
export class WebpackBundler {
  ...
  async build() {
    ...
    const webpackConfigs = [this.getWebpackConfig('Client')];
    ...
    if (styleResources && Object.keys(styleResources).length) {
      ...
      for (const ext of Object.keys(styleResources)) {
        await Promise.all(
          wrapArray(styleResources[ext]).map(async (p: string) => {
            const styleResourceFiles = await glob(
              path.resolve(this.buildContext.options.rootDir, p),
            );
    ...
    if (options.build.parallel) {
      ...
      PerfLoader.warmupAll({
        dev: options.dev,
        resolveModule: (id: any) => tryResolve(id, __filename) || id,
      });
    ...
    await runner(this.compilers, (compiler: any) =>
      this.webpackCompile(compiler),
    );
  }
  ...
  async webpackCompile(compiler: {
    ...
    hooks: {
        ...
        tap: (
          arg0: string,
          arg1: { (stats: any): Promise<void>; (): void },
        ) => void;
      };
    };
    watch: (arg0: any, arg1: (err: any) => void) => any;
    run: () => any;
  }) {
    ...
    compiler.hooks.done.tap('load-resources', async (stats: any) => {
  ...
  async webpackDev(compiler: webpack.ICompiler) {
    ...
    await this.buildContext.nuxt.callHook(
  ...
  async middleware(
    req: { url: any; originalUrl?: any },
    res: any,
    next: () => void,
  ) {
    const name = isModernRequest(req, this.buildContext.options.modern)
      ? 'modern'
      : 'client';
  ...
  async unwatch() {
    await Promise.all(
      this.compilersWatching.map((watching: { close: () => any }) =>
        watching.close(),
      ),
    );
  }
