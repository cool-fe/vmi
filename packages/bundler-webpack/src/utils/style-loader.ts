...
export default class StyleLoader {
  ...
  styleResource(ext) {
    ...
    const patterns = wrapArray(extResource).map((p) =>
  ...
  apply(ext, loaders = []) {
    ...
    const customLoaders = []
      .concat(this.postcss(), this.normalize(loaders), this.styleResource(ext))
      .filter(Boolean);
    ...
    return [
      // This matches <style module>
      {
        ...
        use: this.perfLoader
          .css()
          .concat(this.cssModules(cssModules), customLoaders),
      },
