...
export default class WebpackBaseConfig {
  constructor(builder) {
    ...
    this.resolveModule = (id) =>
      tryResolve(id, [this.buildContext.options.rootDir, __dirname]) || id;
  }
  ...
  getBabelOptions() {
    ...
    if (corejsVersion === 'auto') {
      try {
        corejsVersion = Number.parseInt(
          requireModule('core-js/package.json', rootDir).version.split('.')[0],
        );
      } catch (_err) {
        corejsVersion = 2;
      }
    ...
    if (![2, 3].includes(corejsVersion)) {
      consola.warn(
        `Invalid corejs version ${corejsVersion}! Please set "build.corejs" to either "auto", 2 or 3.`,
      );
      corejsVersion = 2;
    }

    const defaultPreset = [
      this.resolveModule('@nuxt/babel-preset-app'),
      {
        corejs: {
  ...
  getFileName(key) {
    ...
    if (this.dev) {
      ...
      if (hash) {
        consola.warn(
          `Notice: Please do not use ${hash[1]} in dev mode to prevent memory leak`,
        );
      }
    }
    if (
  ...
  env() {
    ...
    if (this.buildContext.buildOptions.aggressiveCodeRemoval) {
      env['typeof process'] = JSON.stringify(
        this.isServer ? 'object' : 'undefined',
      );
      env['typeof window'] = JSON.stringify(
        !this.isServer ? 'object' : 'undefined',
      );
      env['typeof document'] = JSON.stringify(
        !this.isServer ? 'object' : 'undefined',
      );
    }
  ...
  output() {
    ...
    return {
      ...
      publicPath: isUrl(publicPath)
        ? publicPath
        : urlJoin(router.base, publicPath),
    };
  }
  ...
  resolve() {
    ...
    const webpackModulesDir = ['node_modules'].concat(
      this.buildContext.options.modulesDir,
    );
    ...
    const resolvePlugins = [PnpWebpackPlugin].concat(
      resolvePath.map((p) => PnpWebpackPlugin.moduleLoader(p)),
    );
  ...
  alias() {
    return {
      ...
      'vue-meta': this.resolveModule(
        `vue-meta${this.isServer ? '' : '/dist/vue-meta.esm.browser.js'}`,
      ),
    };
  }
  ...
  extendConfig(config) {
    ...
    if (typeof extend === 'function') {
      const extendedConfig =
        extend.call(this.builder, config, {
      ...
      if (typeof devtool === 'string' && pragma.test(devtool)) {
        ...
        consola.warn(
          `devtool has been normalized to ${extendedConfig.devtool} as webpack documented value`,
        );
      }
