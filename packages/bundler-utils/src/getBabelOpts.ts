  ...
  return {
    ...
    cacheDirectory:
      process.env.BABEL_CACHE !== 'none'
        ? winPath(`${prefix}/.vmi/.cache/babel-loader`)
        : false,
  };
}
...
export function getBabelOpts({
  ...
  return {
    ...
    presets: [
      [require.resolve('@vmi/babel-preset-umi/app'), presetOpts],
...
export function getBabelDepsOpts({
  ...
  return {
    ...
    presets: [
      [
        require.resolve('@vmi/babel-preset-umi/dependency'),
        {
          nodeEnv: env,
          dynamicImportNode:
