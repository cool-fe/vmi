...
export default class WebpackClientConfig extends WebpackBaseConfig {
  ...
  get devtool() {
    ...
    const noUnsafeEval =
      scriptPolicy && !scriptPolicy.includes("'unsafe-eval'");
    return noUnsafeEval
  ...
  minimizer() {
    ...
    if (optimizeCSS) {
      minimizer.push(
        new OptimizeCSSAssetsPlugin(Object.assign({}, optimizeCSS)),
      );
    }
  ...
  plugins() {
    ...
    const {
      buildOptions,
      options: { appTemplatePath, buildDir, modern, render },
    } = this.buildContext;
    ...
    if (modern) {
      const scriptPolicy = this.getCspScriptPolicy();
      const noUnsafeInline =
        scriptPolicy && !scriptPolicy.includes("'unsafe-inline'");
      plugins.push(
